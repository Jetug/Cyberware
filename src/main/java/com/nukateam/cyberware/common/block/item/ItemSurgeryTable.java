...
public class ItemSurgeryTable extends Item {
	...
	@Nonnull
	@Override
	public EnumActionResult onItemUse(ItemStack stack, Player entityPlayer, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
	{
		if (worldIn.isRemote)
		{
		...
		else
		{
			...
			if (entityPlayer.canPlayerEdit(pos, facing, stack) && entityPlayer.canPlayerEdit(blockpos, facing, stack))
			{
				...
				if (flag2 && flag3 && worldIn.getBlockState(pos.down()).isFullyOpaque() && worldIn.getBlockState(blockpos.down()).isFullyOpaque())
				{
					...
					worldIn.playSound((Player)null, pos, soundtype.getPlaceSound(), SoundCategory.BLOCKS, (soundtype.getVolume() + 1.0F) / 2.0F, soundtype.getPitch() * 0.8F);
					--stack.stackSize;
					return EnumActionResult.SUCCESS;
				}
				else
